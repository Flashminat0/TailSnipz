...
const FavouriteItemCard = ({id, src, alt, price, size, brand}) => {
    const [singleProduct, setSingleProduct] = useState({});
    const [isFavourite, setIsFavourite] = useState(false);
    let favCount = 2;

    useEffect(async () => {
        await axios.get('/api/product/fetch-single-product', {params: {productId: id}}).then((results)=>{
            ...
            <div className={"p-3 grid grid-cols-3"}>
                <div className={"col-start-1 col-end-3"}>
                    <div className={"flex flex-col gap-0.5"}>
                        <div className={"flex flex-row gap-1"}>
                            <div className={"font-semibold text-sm"}>${singleProduct.price}</div>
                ...
                <div className={"col-start-3 col-end-4"}>
                    <div className={"flex flex-row justify-end items-center gap-0.5 text-gray-400"}>
                        ...
                        {isFavourite === true ? <>
                            <span className={"text-gray-500 font-medium text-sm"}>{singleProduct.favouriteCount + 1}</span>
                        </> : <>
                            <span className={"text-gray-500 font-medium text-sm"}>{singleProduct.favouriteCount}</span>
                        </>}
                    </div>
                </div>
            </div>
