...
const Uploadphotos = ({
  ...
  useEffect(() => {
    inputList.map((item) => {
      const objURL = URL.createObjectURL(item);
      setImageList((prev) => {
        return [
          ...prev,
          {
  ...
  useEffect(() => {
    ...
    imagesObjList.map((objItem, index) => {
      if (objItem.url == removeItem.url) {
        console.log("FOUND " + index + " : " + removeItem.url);
        imagesObjList.splice(index, 1);
        setImagesObjList((prev) => {
          return [...prev];
        });
      }
